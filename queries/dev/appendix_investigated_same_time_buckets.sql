with base as (
    ...
    where s.is_true_onboarding_metric = true
      ...
      and s.squad_other_hires_within_30_days is not null
    group by 1, 2, 3, 4, 5
),
buckets as (
    select
        case
            when squad_other_hires_within_30_days = 0 then '0'
            when squad_other_hires_within_30_days = 1 then '1'
            when squad_other_hires_within_30_days = 2 then '2'
            else '3+'
        end as bucket_label,
        case
            when squad_other_hires_within_30_days = 0 then 1
            when squad_other_hires_within_30_days = 1 then 2
            when squad_other_hires_within_30_days = 2 then 3
            else 4
        end as bucket_order,
        wiser_id,
        days_to_first_pr,
