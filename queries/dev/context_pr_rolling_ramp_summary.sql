...
select
    count(*) filter (where days_since_hire >= 30) as hires_with_30_days,
    median(first_30) filter (where days_since_hire >= 30) as median_first_30_days_prs,
    count(*) filter (where days_since_hire >= 60) as hires_with_60_days,
    median(second_30) filter (where days_since_hire >= 60) as median_second_30_days_prs,
    count(*) filter (where days_since_hire >= 90) as hires_with_90_days,
